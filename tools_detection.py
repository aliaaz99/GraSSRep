...
def get_yinitial(X, y_real, len_thresh, coverage_thresh, isRandom=0):
    ...
    # training nodes number:
    print("Number of training nodes:", len(train_indeces))
...
def embLearn(data_in, model_GNN, N_epoch_GNN, gnnLr):
    ...
    model_GNN.train()
    for epoch in range(N_epoch_GNN):
    ...
    with torch.no_grad():
        ...
        y_prob = out.cpu().numpy()  # Move probabilities back to CPU
    ...
    return X, y_true, y_pred, y_prob[:,1], y_initial, confusion_matrix_gnn, emb_GNN_all
...
def rfLearn(X, y_initial, y_true, train_mask, N_tree):
    ...
    return acc_rf, sen_rf, pre_rf, conf_rf, y_pred_rf, y_prob_rf, rf.feature_importances_
