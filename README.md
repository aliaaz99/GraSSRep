...
In this step, we generate one of the cases of the Simulated dataset introduced in the Results section.
The `insertRepeat.py` script is used to generate a reference genome with inserted repeats. The |--name| parameter specifies the name of the dataset, and the --L and --C parameters specifie the length and copy number of the inserted repeat sequences. A directory for the results is created using mkdir -p.
```sh
